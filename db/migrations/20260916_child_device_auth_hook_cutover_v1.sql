  ...
  if v_token is null or v_token !~ '^[0-9A-Fa-f]{64}$' or v_client_type is null then
    return jsonb_build_object(
      'error',jsonb_build_object(
        'http_code',403,
        'message','Valid child-device enrollment is required.'
      )
    );
  end if;
...
grant execute on function public.hook_xiaoai_child_enrollment_before_user_created_v1(jsonb)
...
drop policy if exists "supabase_auth_admin_read_child_device_enrollment_challenges"
  on public.child_device_enrollment_challenges;
...
begin
  ...
  if v_challenge_id is null then
    raise exception 'child-device enrollment unavailable';
  end if;
...
-- Retire the former parallel client-claim path. The Auth hook + trigger is canonical.
